    ...
    public class messengerPostmaster
    {
        ...
        public messengerPostmaster(int maxSyncMessages)
        {
            if (maxSyncMessages > 0)
                this.maxSyncMessages = maxSyncMessages;
            else
                this.maxSyncMessages = 10;

            this.messageBird = new messageBirdWorker(_launchBird);
        }
        #endregion
        ...
        public void sendMessengerMessage(int[] receiverIDs, messengerMessage pMessage)
        {
            ...
            if (receiverIDs.Length > this.maxSyncMessages) // Launch the message bird on a separate thread
                this.messageBird.BeginInvoke(receiverIDs, pMessage, null, null);
            else
                this.messageBird.Invoke(receiverIDs, pMessage);
        }
