                ...
                if (args[4].Length > 0) // Username given
                {
                    if (requestAction == 0 || requestAction == 1) // Alert or ban
                    {
                        if (requestAction == 0) // Alert
                        {
                            ...
                            {
                                if (ObjectTree.Game.Moderation.requestAlert(Session.User.ID, args[4], args[2], args[3]))
                                    postBackMessage = "Alert sent to user.";
                            }
                        }
                        else if (requestAction == 1) // Kick
                        {
                            if (ObjectTree.Game.Moderation.requestKickFromRoom(Session.User.ID, args[4], args[2], args[3]))
                    ...
                    else if (requestAction == 2) // Ban
                    {
                        if (args[2].Length > 0) // Message given
                        {
                            ...
                            if (ObjectTree.Game.Moderation.requestBan(Session.User.ID, args[4], banHours, banIP, banMachine, args[2], args[3]))
            ...
            else // Room action
            {
                if (Session.inRoom)
                {
                    if (requestAction == 0) // Alert
                    {
                        if (args[2].Length > 0) // Message given
                        {
                            if (ObjectTree.Game.Moderation.requestRoomAlert(Session.User.ID, Session.roomID, args[2], args[3]))
                                postBackMessage = "Alert sent to room.";
                        }
                    }
                    else if (requestAction == 1) // Kick
                    {
                        if (ObjectTree.Game.Moderation.requestRoomKick(Session.User.ID, Session.roomID, args[2], args[3]))
