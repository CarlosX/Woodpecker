    ...
    public class roomUnitStatus
    {
        ...
        public bool Updated
        {
            get
            {
                ...
                if (Check != _lastCheckResult)
                    ret = true;
                _lastCheckResult = Check;
        ...
        public bool Valid
        {
            get
            {
                ...
                {
                    if (this.endTime < DateTime.Now.TimeOfDay.TotalSeconds) // Non-persistent status expired
                        return false;
                }

                if (this.Action != null) // Status changes (eg, carry item)
                {
                    if (this.actionEndTime < DateTime.Now.TimeOfDay.TotalSeconds) // Status requires update
                    {
                        string s = this.Name;
                        ...
                        if (this.actionActive)
                            switchSeconds = this.actionSwitch;
                        else
                            switchSeconds = this.actionLength;

                        this.actionActive = !this.actionActive;
                        this.actionEndTime = DateTime.Now.TimeOfDay.TotalSeconds + switchSeconds;
                        _lastCheckResult = !_lastCheckResult;
                    }
                }
        ...
        public roomUnitStatus(string Name, string Data, int secsLifetime, string Action, int secsActionSwitch, int secsActionLength)
        {
            ...
            this.Data = Data;
            if (secsLifetime > 0)
                this.endTime = DateTime.Now.TimeOfDay.TotalSeconds + secsLifetime;

            if (Action != null)
            {
                this.Action = Action;
                this.actionSwitch = secsActionSwitch;
                this.actionLength = secsActionLength;
                this.actionEndTime = DateTime.Now.TimeOfDay.TotalSeconds + secsActionSwitch;
            }
        }
        #endregion
    }
