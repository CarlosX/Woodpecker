...
namespace Woodpecker.Game.Users
{
    ...
    public class loginReactor : Reactor
    {
        ...
        public void TRY_LOGIN()
        {
            ...
            userInformation userDetails = ObjectTree.Game.Users.getUserInfo(Username, true);
            ...
            else
            {
                ...
                Password = ObjectTree.Security.Cryptography.MD5.Hash(Password, userDetails.Username.ToLower()); // Hash the password
                ...
                if (userDetails.Password == Password) // All details match!
                {
                    ...
                    string banReason = "";
                    if (ObjectTree.Game.Moderation.isBanned(Session.User.ID, out banReason))
                    {
                    ...
                    ObjectTree.Sessions.destroySessions(Session.User.ID); // Destroy previous sessions
                    ...
                    ObjectTree.Game.Users.addUserSession(this.Session);
