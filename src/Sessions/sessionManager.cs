...
namespace Woodpecker.Sessions
{
    public class sessionManager
    {
        ...
        /// </summary>
        private Dictionary<uint, Session> _Sessions = new Dictionary<uint, Session>();
        /// <summary>
        ...
        /// </summary>
        private Thread sessionPingChecker;
        /// <summary>
        ...
        /// </summary>
        private Thread sessionMessageSizeChecker;
        /// <summary>
        /// Incremented every time a session is created.
        /// </summary>
        private uint sessionCounter = 1;
        #endregion
        ...
        public void Clear()
        {
            lock (_Sessions)
            {
                foreach (Session lSession in this._Sessions.Values)
                {
                    lSession.Destroy();
                }
            }
        ...
        {
            this.sessionPingChecker = new Thread(new ThreadStart(this.checkSessionPings));
            try { this.sessionPingChecker.Start(); }
            catch { }
        ...
        {
            try { this.sessionPingChecker.Abort(); }
            catch { }
        ...
        private void checkSessionPings()
        {
            ...
            while (true)
            {
                //DateTime dtMax = DateTime.Now.AddMinutes(this.maxIdleTime);
                
                lock (_Sessions)
                {
                    foreach (Session lSession in _Sessions.Values)
                    {
        ...
        {
            this.sessionMessageSizeChecker = new Thread(new ThreadStart(this.checkSessionReceivedBytes));
            this.sessionMessageSizeChecker.Priority = ThreadPriority.Lowest;
            try { this.sessionMessageSizeChecker.Start(); }
            catch { }
        ...
        {
            try { this.sessionMessageSizeChecker.Abort(); }
            catch { }
        ...
        private void checkSessionReceivedBytes()
        {
            ...
            while (true)
            {
                ...
                {
                    ObjectTree.Net.Game.blackListIpAddress(lDirtySession.ipAddress);
        ...
        public void listSessions()
        {
            ...
            Dictionary<uint, Session>.Enumerator lSessions = this._Sessions.GetEnumerator();
        ...
        public void destroySession(uint sessionID)
        {
            ...
            {
                Session dSession = this._Sessions[sessionID];
                dSession.Destroy();
                dSession = null;
        ...
        {
            uint sessionID = ObjectTree.Game.Users.getUserSessionID(userID);
            if (sessionID > 0)
